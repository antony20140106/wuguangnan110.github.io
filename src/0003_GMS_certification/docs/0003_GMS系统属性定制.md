...
目前MTK平台有两个项目，M8和M5x，需要共用同一套代码，需要从RC、kernel、lk三个方面做产品兼容。
...
![0003_.png](images/0003_yuanlitu.png)
...
on init
#机型名称
    setprop ro.product.model M5x
    setprop ro.product.name M5x
    setprop ro.product.device M5x
	setprop	ro.product.board M5x
```
* selinux报错如下：
...
(210510_16:12:30.721)[    7.987986] <1>.(1)[1:init]init 24: [7959][0]processing action (init) from (/vendor/xxxxxdroid/rc_file/init.mt6762_M5x_V0.rc:33)
...
```
device/a133/a133.mk
70:    xxxxxdroid/device/a133/init.xxxxxdroid.rc:root/init.xxxxxdroid.rc \
71:    xxxxxdroid/device/a133/init.xxxxxdroid.A35.rc:root/init.xxxxxdroid.A35.rc \
72:    xxxxxdroid/device/a133/init.xxxxxdroid.A660.rc:root/init.xxxxxdroid.A660.rc \
73:    xxxxxdroid/device/a133/init.xxxxxdroid.A660.rc:root/init.xxxxxdroid.P100.rc \
74:    xxxxxdroid/device/a133/init.xxxxxdroid.QR500.rc:root/init.xxxxxdroid.QR500.rc \
75:    xxxxxdroid/device/a133/init.xxxxxdroid.AF5.rc:root/init.xxxxxdroid.AF5.rc \
76:    xxxxxdroid/device/a133/init.xxxxx.usb.rc:root/init.xxxxx.usb.rc \
```
## rc定制属性类型
* 目前M5x需要定制的变量如下：
...
属性 | 说明 | 值
---|---|---
ro.build.product | 已经过时，现在使用“ro.product.device” | 值同“ro.product.device”即可 M5x
ro.product.device | 设备名称 | PAYPHONEM5x
ro.product.model | 型号名称 | M5x
ro.product.name | 产品名称 | M5x
ro.product.board | 采用的底板/处理器 | M5x
ro.product.manufacturer | 制造商名称 | xxx

另外还有一些属性如下：

```
	setprop	ro.product.odm.device M5x
	setprop	ro.product.odm.model M5x
	setprop	ro.product.odm.name M5x
	
    setprop ro.product.product.device PAYPHONEM5x
	setprop	ro.product.product.model M5x
	setprop	ro.product.product.name M5x
	
	setprop	ro.product.system.device PAYPHONEM5x
	setprop	ro.product.system.model M5x
	setprop	ro.product.system.name M5x
	
	setprop	ro.product.vendor.device PAYPHONEM5x
	setprop	ro.product.vendor.model M5x
	setprop	ro.product.vendor.name M5x
	
	setprop	ro.product.system_ext.device PAYPHONEM5x
	setprop	ro.product.system_ext.model M5x
	setprop	ro.product.system_ext.name M5x
```
...
```
    setprop ro.product.device PAYPHONEM5x
	setprop ro.product.model M5x
	setprop ro.product.name M5x
	setprop	ro.product.board M5x
	setprop ro.product.manufacturer xxx
```

* ro.product.name定义如下：`device/mediateksample/k62v1_64_xxxxx/k62v1_64_xxxxx.mk`:

```
ro.product.name:
PRODUCT_NAME := k62v1_64_xxxxx
```
* ro.product.brand、ro.product.model、ro.product.manufacture、ro.product.devicer定义如下：`device/mediateksample/k62v1_64_xxxxx/vnd_k62v1_64_xxxxx.mk`
```
这边只是初步定义，可以全部改变，但是最后是在接下来讲的cpp文件里面改：
MTK_BASE_PROJECT := k62v1_64_xxxxx
PRODUCT_MANUFACTURER := alps
PRODUCT_DEVICE := $(strip $(MTK_BASE_PROJECT))
PRODUCT_MODEL := k62v1_64_xxxxx
PRODUCT_BRAND := alps
```
...
static void property_initialize_ro_product_props() {
    ...
    const char* RO_PRODUCT_PROPS[] = {
			/* [FEATURE]-Modify-BEGIN by (xxx@xxxxx.com), 2021/05/10, don't set ro.product property auto*/
            //"brand", "device", "manufacturer", "model", "name",  注释掉这里就好了
			"brand",
			/* [FEATURE]-Modify-END by (xxx@xxxxx.com), 2021/05/10, don't set ro.product property auto*/
    };
...
```
Setting product property ro.product.brand to 'xxx' (from ro.product.odm.brand)
```
#### 1.2ro.product重新赋值
既然在property_initialize_ro_product_props中已经将赋值全部删除，就需要重新赋值，修改如下`xxxxxdroid/device/xxxxxdroid_common.mk`：

```
赋值这个属性必须放在root路径下，否则会有selinux权限报错：
ifeq ($(strip $(POS_OS_PLATFORM)), MT6762)
PRODUCT_COPY_FILES += \
        xxxxxdroid/device/build.prop/build_A80.prop:vendor/xxxxxdroid/build_A80.prop \
        xxxxxdroid/device/build.prop/build_IM30.prop:vendor/xxxxxdroid/build_IM30.prop \
        xxxxxdroid/device/etc/pms_app_grant_permission_list.txt:vendor/xxxxxdroid/pms_app_grant_permission_list.txt \
        xxxxxdroid/device/rc_file/M5x/init.recovery.M5x_V0.rc:root/init.recovery.M5x_V0.rc \
        xxxxxdroid/device/rc_file/M5x/init.mt6762_M5x_V0.rc:root/init.mt6762_M5x_V0.rc \
        
init.mt6762_M5x_V0.rc: 重新赋值        
on init
	setprop	ro.build.product M5x
...
只展现相关属性
system/build.prop：
ro.product.system.brand=xxx
ro.product.system.device=xxxM5x
ro.product.system.manufacturer=xxx
ro.product.system.model=M5x
ro.product.system.name=M5x
ro.build.product=M5x

system/system_ext/build.prop:
ro.product.system_ext.brand=alps
ro.product.system_ext.device=k62v1_64_xxxxx
ro.product.system_ext.manufacturer=alps
ro.product.system_ext.model=k62v1_64_xxxxx
ro.product.system_ext.name=k62v1_64_xxxxx

vendor/build.prop:
ro.product.vendor.brand=alps
ro.product.vendor.device=k62v1_64_xxxxx
ro.product.vendor.manufacturer=alps
ro.product.vendor.model=k62v1_64_xxxxx
ro.product.vendor.name=k62v1_64_xxxxx

vendor/odm/etc/build.prop：
ro.product.odm.brand=alps
ro.product.odm.device=k62v1_64_xxxxx
ro.product.odm.manufacturer=alps
ro.product.odm.model=k62v1_64_xxxxx
ro.product.odm.name=k62v1_64_xxxxx

product/build.prop：
ro.product.product.brand=alps
ro.product.product.device=k62v1_64_xxxxx
ro.product.product.manufacturer=alps
ro.product.product.model=k62v1_64_xxxxx
ro.product.product.name=k62v1_64_xxxxx
```
#### 2. build.prop生成过程
...
而不是device/mediateksample/k62v1_64_xxxxx/system.prop
　　到此，prop文件生成完毕！
```
...
ro.build.display.id=k62v1_64_xxxxx-userdebug 11 RP1A.200720.011 PayDroid1100BirchV15100 release-keys
...
#
# from xxxxxdroid/device/build.prop/build_k62v1_64_xxxxx.prop
#
ro.config.vc_call_vol_default=5
# end of xxxxxdroid/device/build.prop/build_k62v1_64_xxxxx.prop
...
ifeq ($(PRODUCT_RETROFIT_DYNAMIC_PARTITIONS),true)
	...
	#[FEATURE]-Add-BEGIN by (xxx@xxxxx.com), 2021/05/11, delete prop ro.product.board set in vendor's prop file
	#$(hide) echo ro.product.board="$(TARGET_BOOTLOADER_BOARD_NAME)">>$@
	#[FEATURE]-Add-END by (xxx@xxxxx.com), 2021/05/11, delete prop ro.product.board set in vendor's prop file
...
2.调用buildinfo_common.sh写vendor分区专有属性，ro.vendor.build类
	...
	#[FEATURE]-Add-BEGIN by (xxx@xxxxx.com), 2021/05/11, delete prop ro.bootimage.build.fingerprint set in vendor's prop file
	#$(hide) echo ro.bootimage.build.fingerprint="$(BUILD_FINGERPRINT_FROM_FILE)">>$@
	#[FEATURE]-Add-END by (xxx@xxxxx.com), 2021/05/11, delete prop ro.bootimage.build.fingerprint set in vendor's prop file
...
#[FEATURE]-Add-BEGIN by (xxx@xxxxx.com), 2021/05/11, delete ro.${partition}.build.fingerprint set
#echo "ro.${partition}.build.fingerprint=$BUILD_FINGERPRINT"
#[FEATURE]-Add-END by (xxx@xxxxx.com), 2021/05/11, delete ro.${partition}.build.fingerprint set 
...
#[FEATURE]-Add-BEGIN by (xxx@xxxxx.com), 2021/05/11, delete prop ro.product.product set 
...
#[FEATURE]-Add-END by (xxx@xxxxx.com), 2021/05/11, delete prop ro.product.product set
...
ro.product.vendor.brand=xxx
ro.product.vendor.manufacturer=xxx
...
```
xxx/k62v1_64_xxxxx/k62v1_64_xxxxx:11/RP1A.200720.011/PayDroid1100BirchV15100:userdebug/release-keys
```
...
static void property_derive_build_fingerprint() {
    ...
    build_fingerprint += '/';
	//[FEATURE]-Add-BEGIN by (xxx@xxxxx.com), 2021/05/11, set some fingerprint prop for GMS
	...
	build_fingerprint += 'E';
    build_fingerprint += GetProperty("ro.boot.terminal_name", UNKNOWN);
	if (GetProperty("ro.xxxxxdroid.name", UNKNOWN) == "k62v1_64_xxxxx") {
		build_fingerprint += '_';
		build_fingerprint += 'E';
		build_fingerprint += 'E';
		build_fingerprint += 'A';
	}
	//[FEATURE]-Add-END by (xxx@xxxxx.com), 2021/05/11, set some fingerprint prop for GMS
    build_fingerprint += ':';
...
```
我们知道fingerprint的属性有三个地方，我目前的做法是前两个地方的都不用了，注释掉，将所有属性定义都放在`property_derive_build_fingerprint`中，这样能够灵活的区分是M5x还是M8，或者EEA版本还是非EEA版本。
