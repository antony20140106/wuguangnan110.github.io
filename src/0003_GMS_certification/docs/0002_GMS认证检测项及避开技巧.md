...
* (4)设置中显示的Android安全补丁日期与ro.build.security_patch.xxxxx属性值一致。
...
static void property_derive_build_fingerprint() {
    ...
    std::string error;
    //[FEATURE]-Add-BEGIN by (qumy@xxxxx.com), 2021/08/11, modify pro ro.build.version.incremental
    ...
    }else if(GetProperty("ro.boot.terminal_name", UNKNOWN) == "M5x"){
    if(GetProperty("ro.xxxxxdroid.name", UNKNOWN) == "k62v1_64_xxxxx"){
    ...
    //[FEATURE]-Add-END by (qumy@xxxxx.com), 2021/08/11,
    ...
    //[FEATURE]-Add-BEGIN by (xxx@xxxxx.com), 2021/08/06, for eea product
    if (GetProperty("ro.xxxxxdroid.name", UNKNOWN) == "k62v1_64_xxxxx_eea") {
        build_fingerprint += '_';
    build_fingerprint += 'E';
    build_fingerprint += 'E';
    build_fingerprint += 'A';
   ...
   if(GetProperty("ro.boot.terminal_name", UNKNOWN) == "M8") {
    ...
    build_fingerprint += GetProperty("ro.boot.terminal_name", UNKNOWN);
    //[FEATURE]-Add-BEGIN by (xxx@xxxxx.com), 2021/08/06, for eea product for eea,not to add _M8
    // if (GetProperty("ro.xxxxxdroid.name", UNKNOWN) == "k62v1_64_xxxxx_eea" && GetProperty("ro.boot.terminal_name", UNKNOWN) == "M8") {
    //  build_fingerprint += '_';
    //  build_fingerprint += 'E';
    //  build_fingerprint += 'E';
    //  build_fingerprint += 'A';
    // }
    //[FEATURE]-Add-END by (xxx@xxxxx.com), 2021/08/06, for eea product
...
```
M5x:
[ro.build.fingerprint]: [xxx/M5x_EEA/PAYPHONEM5x:11/RP1A.200720.011/2106082124:user/release-keys]

M8:
[ro.build.fingerprint]: [xxx/M8_EEA/PAYPHONEM5x:11/RP1A.200720.011/2106082124:user/release-keys]
```
...
```diff
 ...
 .../apps/Google/google/products/gms.mk        |   1 -
  .../rc_file/M5x/init.mt6762_common_M5x.rc     |  71 ++++-----
 .../rc_file/M5x/init.mt6762_common_M5x_eea.rc |  64 ++++----
...
-    # [BUGFIX]-BEGIN by (xxx@xxxxx.com), 2021/12/22 for keep the security patch date the same as at the time of certification,and define in rc file
-    #  PLATFORM_SECURITY_PATCH := 2021-11-05
-    # [BUGFIX]-END by (xxx@xxxxx.com), 2021/12/22 for keep the security patch date the same as at the time of certification,and define in rc file
+      PLATFORM_SECURITY_PATCH := 2021-11-05
 endif
...
+#[FEATURE]-Add-BEGIN by (lib@xxxxx.com) 2021/12/21 add the xxxxx security patch
+#echo "ro.build.version.security_patch=$PLATFORM_SECURITY_PATCH"
+echo "ro.build.version.security_patch_xxxxx=$PLATFORM_SECURITY_PATCH"
+#[FEATURE]-Add-END by (lib@xxxxx.com) 2021/12/21
 echo "ro.build.version.base_os=$PLATFORM_BASE_OS"
 echo "ro.build.version.min_supported_target_sdk=$PLATFORM_MIN_SUPPORTED_TARGET_SDK_VERSION"
 echo "ro.build.date=`$DATE`"

 --- a/xxxxxdroid/device/rc_file/M5x/init.mt6762_common_M5x.rc
+++ b/xxxxxdroid/device/rc_file/M5x/init.mt6762_common_M5x.rc
...
[ro.build.version.security_patch_xxxxx]: [2021-11-05]
...
![0002_meta.png](images/0002_metano.png)

* 原因：

meta_init.mt6762_M5x_V2.rc文件中未设置ro.build.version.security_patch属性，导致meta系统无法启动。
