...
目前mp2721在检测到SNK attach后，会反复轮询bc12检测是否完成，完成后会通知xxx_charger并唤醒充电线程进行plug in配置充电，并5s轮询一次充电状态。
...
* xxx_charger调用，首先线程会判断primary_chg有没有注册，注册了才会跑线程，不然一直阻塞：
```C++
...
static bool charger_init_algo(struct xxx_charger *info)
{
...
```C++
xxx_charger.c
static bool xxx_is_charger_on(struct xxx_charger *info)
{
	int chr_type;
...
xxx_charger_class.c
int xxx_charger_dev_get_charger_type(struct charger_device *charger_dev)
{
...
EXPORT_SYMBOL(xxx_charger_dev_get_charger_type);
```
...
* xxx_charger采用直接轮询charger ic状态的方式，更加方便，而且现在把psy设备注册放到xxx_charger端，如果状态有变化直接调用`power_supply_changed`上报状态，无需再转手。
```C++
static void xxx_charger_check_status(struct xxx_charger *info)
{
	static char chg_status = 0;

	chg_status = xxx_charger_dev_get_charging_status(info->chg1_dev);
	...
	xxx_charger_alarm_timer_func(struct alarm *alarm, ktime_t now)
{
	...
	container_of(alarm, struct xxx_charger, charger_timer);
...
static void xxx_charger_start_timer(struct xxx_charger *info)
{
			...
			unsigned long pm_event, void *unused)
{
	...
	info = container_of(notifier,
		struct xxx_charger, pm_notifier);
...
* 2、定时器超时函数`xxx_charger_alarm_timer_func`，根据`is_suspend`状态，这里会持锁唤醒系统。
...
[10990.193251] xxx_CHG: charger_pm_event: enter PM_SUSPEND_PREPARE //调用suspend_prepare做进一步suspend前期准备工作，准备控制台，冻结内核线程等
...
[10990.258791] [xxx_authinfo]: gpio_sleep_sp, en=1
[10990.258791]
[10990.258812] xxx_BMS:bms_suspend. secs = 572700
[10990.258838] xxx_base_detect_suspend
...
[10990.385117] xxx_CHG: xxx_charger_alarm_timer_func: alarm timer timeout //定时器超时函数，这里会持锁唤醒系统
...
[10990.506155] xxx_base_detect_resume
[10990.506957] xxx_BAT: xxx_battery_resume: pre_soc: 100 soc: 100
...
[10990.577217] xxx_BAT: [status:Full, health:Good, present:1, tech:Li-ion, capcity:100,cap_rm:5045 mah, vol:4301 mv, temp:29, curr:0 ma, ui_soc:100]
...
[10990.804820] xxx_CHG: charger_pm_event: enter PM_POST_SUSPEND //唤醒后进入PM_POST_SUSPEND阶段，就是退出休眠
[10990.810534] xxx_CHG: charger_pm_event: alarm timeout, wake up charger //并唤醒充电线程
[10990.817061] Resume caused by IRQ 173, pm8xxx_rtc_alarm
[10990.817305] xxx_CHG: xxx_is_charger_on chr_type = [DCP] last_chr_type = [DCP]
...
static void xxx_wait_bc12_start_timer(void)
{
	struct timespec time, time_now;
	ktime_t ktime;
	int ret = 0;
...
static enum alarmtimer_restart
	xxx_wait_bc12_alarm_timer_func(struct alarm *alarm, ktime_t now)
{
	uint8_t type = TYPEC_UNATTACHED;
	type = g_info->typec_state;

	g_info->chr_type = xxx_charger_dev_get_charger_type(g_info->chg1_dev);
	...
	if (g_info->chr_type != POWER_SUPPLY_TYPE_UNKNOWN ||
		...
		pr_err("restart xxx_wait_bc12_alarm_timer_func\n");
		g_info->wait_bc12_count++;
		xxx_wait_bc12_start_timer();
	}
out:
...
static void xxx_charger_init_timer(struct xxx_charger *info)
{
	alarm_init(&info->charger_timer, ALARM_BOOTTIME,
			xxx_charger_alarm_timer_func);
	xxx_charger_start_timer(info);

	alarm_init(&info->wait_bc12_timer, ALARM_BOOTTIME,
	xxx_wait_bc12_alarm_timer_func); //初始化timer
...
void handle_typec_attach_dettach(bool en)
{
	...
	/* The thread notifies the xxx charger that the device is connected */
	g_info->attach = en;
	if (en) {
...
```C++
static void xxx_wait_bc12_work(struct work_struct *work)
{
	...
	if (g_info->chr_type != POWER_SUPPLY_TYPE_UNKNOWN ||
		...
		pr_err("restart xxx_wait_bc12 dwork\n");
		g_info->wait_bc12_count++;
		cancel_delayed_work(&g_info->wait_bc12_dwork);
...
```C++
[  859.346234] xxx-pd-manager soc:xxx_pd_manager: pd_tcp_notifier_call event = 10
[  859.364617] xxx-pd-manager soc:xxx_pd_manager: pd_tcp_notifier_call sink vbus 5000mV 100mA type(0x01)
[  859.377953] xxx-pd-manager soc:xxx_pd_manager: pd_tcp_notifier_call event = 14
[  859.385984] xxx-pd-manager soc:xxx_pd_manager: pd_tcp_notifier_call Charger plug in, polarity = 0
...
[  859.417091] xxx-pd-manager soc:xxx_pd_manager: usb_dwork_handler Device
...
[  859.551367] restart xxx_wait_bc12 dwork
...
[  859.614515] restart xxx_wait_bc12 dwork
[  859.668206] chg_type=0, count=3
[  859.671392] restart xxx_wait_bc12 dwork
...
[  859.732692] restart xxx_wait_bc12 dwork
...
[  859.829601] xxx_is_charger_on chr_type = 4
```
