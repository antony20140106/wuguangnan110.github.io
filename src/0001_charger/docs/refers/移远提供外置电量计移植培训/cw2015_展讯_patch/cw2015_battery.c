...
*/
static unsigned char config_info[SIZE_BATINFO] = {
    0x15, 0x42, 0x60, 0x59, 0x52, 0x58, 0x4D, 0x48,
...
/*
	int if_charging = 0;
	if(check_charging_state(USB_CHARGING_FILE) == 1 || check_charging_state(DC_CHARGING_FILE) == 1)
	{
...
int sprd_get_cw2015_capacity(void)
{
	int ret;
	ret = cw_get_capacity(cw2015_bat);
	return ret;
}
EXPORT_SYMBOL(sprd_get_cw2015_capacity);
int sprd_get_cw2015_vol(void)
{
	int ret;
	ret = cw_get_voltage(cw2015_bat);
	return ret;
}
EXPORT_SYMBOL(sprd_get_cw2015_vol);
...
//static struct regulator *cw_vcc_i2c;
//struct cw_battery *cw_bms_gbl;
...
static int cw2015_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //struct device *dev;
	//cw_printk("cw2015_probe \n");
	printk(KERN_ERR "cw2015_probe...\n");
	...
	psy_cfg.supplied_to = cw2015_charger_supplied_to;
	psy_cfg.num_supplicants = ARRAY_SIZE(cw2015_charger_supplied_to);
...
int  bm_cw2015_init(void)
{
		
	printk(KERN_ERR"bm_cw2015_init\n");
	return i2c_add_driver(&cw2015_driver);
}

void  bm_cw2015_exit(void)
{
